...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "LOWPASS.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("LOWPASS");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow1 = 1.0f / std::tan(fConst0 * float(fEntry0));
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fVec0[0] = fTemp0;
			fRec1[0] = -(fSlow3 * (fSlow2 * fRec1[1] - (fTemp0 + fVec0[1])));
	...
	#define FAUST_FILE_NAME "LOWPASS.dsp"
...
LOWPASS::LOWPASS() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void LOWPASS::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void LOWPASS::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float LOWPASS::getParamValue(const std::string& path)
{
