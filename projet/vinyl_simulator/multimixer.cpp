...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "multimixer.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("multimixer");
		ui_interface->addHorizontalSlider("NoiseLevel", &fHslider0, FAUSTFLOAT(0.02f), FAUSTFLOAT(0.0f), FAUSTFLOAT(0.5f), FAUSTFLOAT(0.001f));
		ui_interface->addHorizontalSlider("Wow Frequency", &fHslider1, FAUSTFLOAT(1.0f), FAUSTFLOAT(0.1f), FAUSTFLOAT(5.0f), FAUSTFLOAT(0.01f));
		ui_interface->addNumEntry("freq", &fEntry0, FAUSTFLOAT(2e+02f), FAUSTFLOAT(5e+01f), FAUSTFLOAT(1e+03f), FAUSTFLOAT(1e+02f));
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = 4.656613e-10f * float(fHslider0);
		float fSlow1 = float(fHslider1);
		float fSlow2 = 31.415926f * fSlow1;
		float fSlow3 = 18.849556f * fSlow1;
		float fSlow4 = 6.2831855f * fSlow1;
		float fSlow5 = 1.0f / std::tan(fConst0 * float(fEntry0));
		float fSlow6 = 1.0f - fSlow5;
		float fSlow7 = 1.0f / (fSlow5 + 1.0f);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			iRec0[0] = 1103515245 * iRec0[1] + 12345;
			float fTemp0 = float(input0[i0]) * (0.5f * (2.0f * std::sin(fSlow4 * float(input1[i0])) + 0.6666667f * std::sin(fSlow3 * float(input2[i0])) + 0.4f * std::sin(fSlow2 * float(input3[i0]))) + 1.0f);
			fVec0[0] = fTemp0;
			fRec1[0] = -(fSlow7 * (fSlow6 * fRec1[1] - (fTemp0 + fVec0[1])));
			output0[i0] = FAUSTFLOAT(fRec1[0] + fSlow0 * float(iRec0[0]));
			iRec0[1] = iRec0[0];
			fVec0[1] = fVec0[0];
			fRec1[1] = fRec1[0];
		}
	...
	#define FAUST_FILE_NAME "multimixer.dsp"
	...
	FAUST_ADDHORIZONTALSLIDER("NoiseLevel", fHslider0, 0.02f, 0.0f, 0.5f, 0.001f);
	FAUST_ADDHORIZONTALSLIDER("Wow Frequency", fHslider1, 1.0f, 0.1f, 5.0f, 0.01f);
	FAUST_ADDNUMENTRY("freq", fEntry0, 2e+02f, 5e+01f, 1e+03f, 1e+02f);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, NoiseLevel, "NoiseLevel", fHslider0, 0.02f, 0.0f, 0.5f, 0.001f) \
		p(HORIZONTALSLIDER, Wow_Frequency, "Wow Frequency", fHslider1, 1.0f, 0.1f, 5.0f, 0.01f) \
		p(NUMENTRY, freq, "freq", fEntry0, 2e+02f, 5e+01f, 1e+03f, 1e+02f) \
...
multimixer::multimixer() : AudioStream(FAUST_INPUTS, new audio_block_t*[FAUST_INPUTS])
{
#ifdef NVOICES
    int nvoices = NVOICES;
...
void multimixer::updateImp(void)
{
#if MIDICTRL
...
void multimixer::update(void) { updateImp<FAUST_INPUTS, FAUST_OUTPUTS>(); }

void multimixer::setParamValue(const std::string& path, float value)
{
    fUI->setParamValue(path, value);
}

float multimixer::getParamValue(const std::string& path)
{
